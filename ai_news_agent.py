...
"""
    ...
    def save_digest(self, news_items: List[NewsItem], summary: str = ""):
        """Save text digest to output folder."""
    ...
    def send_notifications(self, news_items: List[NewsItem], summary: str = ""):
        """Send email and/or Slack notifications."""
        
        # Send email if configured
        if self.config.get("email_config", {}).get("enabled"):
            self._send_email(news_items)
        
        # Send Slack if configured
        if self.config.get("slack_config", {}).get("enabled"):
            self._send_slack_message(news_items, summary)
    
    def _send_email(self, news_items: List[NewsItem]):
        """Send email digest."""
        email_config = self.config.get("email_config", {})
        method = email_config.get("method", "file")
        
        if method == "smtp":
            self._send_via_smtp(email_config, news_items)
        elif method == "api":
            self._send_via_api(email_config, news_items)
        else:
            logging.info("Email method not configured for sending")
    
    def _send_via_smtp(self, config: Dict, news_items: List[NewsItem]):
        """Send email via SMTP."""
        try:
            ...
            msg = MIMEText(self.generate_email_text(news_items), "plain")
    ...
    def _send_via_api(self, config: Dict, news_items: List[NewsItem]):
        ...
        try:
            ...
            if provider.lower() == "sendgrid":
                self._send_via_sendgrid(api_key, from_email, to_email, news_items)
            elif provider.lower() == "mailgun":
                self._send_via_mailgun(api_config, from_email, to_email, news_items)
            else:
    ...
    def _send_via_sendgrid(self, api_key: str, from_email: str, to_email: str, news_items: List[NewsItem]):
        ...
        try:
            ...
            data = {
                ...
                "content": [
                    {"type": "text/plain", "value": self.generate_email_text(news_items)}
                ]
            }
    ...
    def _send_via_mailgun(self, config: Dict, from_email: str, to_email: str, news_items: List[NewsItem]):
        ...
        try:
            ...
            data = {
                ...
                "to": to_email,
                "subject": f"🤖 AI News Digest - {self.run_date.strftime('%B %d, %Y')}",
                "text": self.generate_email_text(news_items)
            }
    ...
    def run(self):
        ...
        try:
            ...
            # Send notifications
            self.send_notifications(news_items, summary)
